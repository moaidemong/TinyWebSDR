...
- **Producer**: The `src/core_producer.py` component that interfaces directly with RTL-SDR hardware, reads IQ samples, performs signal processing (windowing, FFT, dB conversion), and writes results to shared memory.

- **Gateway**: The `src/ws_server.py` component that reads processed data from shared memory and forwards it to browser clients via WebSocket. Acts as the bridge between the producer and web frontend.
